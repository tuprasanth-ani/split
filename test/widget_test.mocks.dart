...
abstract class BaseDatabaseService extends ChangeNotifier {
  ...
  Future<bool> deleteExpense(String expenseId, [String? groupId]);
}
...
void main() {
  group('Splitzy App Widget Tests', () {
    ...
    group('Database Service Tests', () {
      ...
      testWidgets('can delete expense', (WidgetTester tester) async {
        final result = await mockDatabaseService.deleteExpense('expense1', 'group1');

        expect(result, true); // Verify successful deletion
      });

      testWidgets('can delete expense without group', (WidgetTester tester) async {
        final result = await mockDatabaseService.deleteExpense('expense1');

        expect(result, true); // Verify successful deletion without group
      });

      testWidgets('handles delete expense errors correctly', (WidgetTester tester) async {
        mockDatabaseService.setShouldThrowError(true);

        expect(() => mockDatabaseService.deleteExpense('expense1', 'group1'), throwsException);
      });
    });
